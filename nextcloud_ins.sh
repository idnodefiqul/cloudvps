...
show_summary() {
    ...
    echo "1. Pastikan domain $DOMAIN sudah mengarah ke IP server ini"
    echo "2. Buka browser dan akses URL di atas"
    echo "3. Buat akun admin Nextcloud"
    if [[ $DATABASE != "sqlite" ]]; then
        echo "4. Gunakan informasi database di atas untuk konfigurasi"
    else
        echo "4. Pilih SQLite sebagai database"
    fi
    echo "5. Selesaikan setup melalui web interface"
    echo
}
